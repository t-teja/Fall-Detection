...
public class SensorDataCollector implements SensorEventListener {
    ...
    private void checkForFall(SensorDataWindow window) {
        if (window.accelerometerData.isEmpty()) return;
        
        // Simple fall detection algorithm
        // Look for sudden acceleration followed by free fall
        ...
        float maxMagnitude = calculateMax(magnitudes);
        float minMagnitude = calculateMin(magnitudes);
        
        // Fall detection thresholds
        float impactThreshold = 20.0f; // High acceleration (impact)
        float freeFallThreshold = 2.0f; // Low acceleration (free fall)
        
        boolean hasImpact = maxMagnitude > impactThreshold;
        boolean hasFreeFall = minMagnitude < freeFallThreshold;
        
        if (hasImpact && hasFreeFall) {
            float confidence = Math.min(1.0f, (maxMagnitude - impactThreshold) / 10.0f);
            if (dataListener != null) {
                dataListener.onFallDetected(confidence);
            }
        }
    }
